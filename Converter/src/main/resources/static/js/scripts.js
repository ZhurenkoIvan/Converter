...
//host
const hostCurrency = 'http://localhost:8080/api/exchangeRates'
const hostStats = 'http://localhost:8080/api/stats'
const hostInfo = 'http://localhost:8080/api/transactionInfo'
...
//Событие обновления базы данных
 btnUpdateBase.addEventListener('click', async function () {
  const response = await fetch(hostCurrency, {
    method: 'POST',
    body: JSON.stringify({
    }),
    headers: {
